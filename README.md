...
- `app/task1/page.tsx`：使用 viem 连接 Sepolia 测试网，查询指定地址的测试 ETH 余额，并展示发送 ETH、ERC-20 查询、事件监听和 token 转账子任务。
- `app/task1/page2.tsx`：使用 WalletClient 通过浏览器钱包发送 Sepolia ETH。
- `app/task1/page3.tsx`：调用 ERC-20 合约的 `balanceOf` 方法，查询指定地址的 token 余额。
- `app/task1/page4.tsx`：监听 ERC-20 合约的 `Transfer` 事件。
- `app/task1/page5.tsx`：使用 WalletClient 调用 ERC-20 `transfer` 方法发送 token。
...
app/
  ...
  task1/
    ...
    page4.tsx       viem 任务 1 子任务：ERC-20 Transfer 事件监听
    page5.tsx       viem 任务 1 子任务：ERC-20 token 转账
...
NEXT_PUBLIC_TASK1_ERC20_CONTRACT_ADDRESS=0x9727B8991A6B9BFDB2ae736FD80e367221F8f1BC
NEXT_PUBLIC_TASK5_TOKEN_RECIPIENT_ADDRESS=0xd1C18624549f755fFb778F19760d8110548937b8
NEXT_PUBLIC_TASK5_DEFAULT_TOKEN_AMOUNT=1
```
...
`app/task1/page4.tsx` 运行在浏览器端，会从 `NEXT_PUBLIC_SEPOLIA_RPC_URL` 和 `NEXT_PUBLIC_TASK1_ERC20_CONTRACT_ADDRESS` 读取事件监听参数。
`app/task1/page5.tsx` 运行在浏览器端，会从 `NEXT_PUBLIC_TASK5_TOKEN_RECIPIENT_ADDRESS` 和 `NEXT_PUBLIC_TASK5_DEFAULT_TOKEN_AMOUNT` 读取 ERC-20 转账默认参数。
